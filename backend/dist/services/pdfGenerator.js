...
exports.generateAnalysisPdf = generateAnalysisPdf;
...
async function getStudentName(studentId) {
    ...
    try {
        ...
        return (_a = user === null || user === void 0 ? void 0 : user.name) !== null && _a !== void 0 ? _a : 'Öğrenci';
    }
    catch {
        return 'Öğrenci';
    }
 ...
 * @returns PDF buffer
 */
async function generateAnalysisPdf(studentId, examId) {
    var _a, _b;
    const eid = Number(examId);
    if (isNaN(eid))
        throw new Error('Geçersiz examId');
    const [studentName, analysis] = await Promise.all([
        getStudentName(studentId),
        (0, analysisService_1.getExamAnalysisForStudent)(studentId, eid),
    ]);
    if (!analysis) {
    ...
    const [multiExam, topicGroups] = await Promise.all([
        (0, analysisService_1.getMultiExamBranchTable)(studentId, 10),
        (0, analysisService_1.getTopicGroupAnalysisForExam)(studentId, eid),
    ]);
    ...
    const assetsFontsDir = path_1.default.join(__dirname, '..', '..', 'assets', 'fonts');
    const assestsFontsDir = path_1.default.join(__dirname, '..', '..', 'assests', 'fonts');
    const fontsDir = fs_1.default.existsSync(assetsFontsDir)
        ? assetsFontsDir
        : fs_1.default.existsSync(assestsFontsDir)
            ? assestsFontsDir
            : assetsFontsDir;
    let fontHeading = 'Helvetica-Bold';
    let fontBody = 'Helvetica';
    try {
        const regularPath = path_1.default.join(fontsDir, 'NotoSans-Regular.ttf');
        const boldPath = path_1.default.join(fontsDir, 'NotoSans-Bold.ttf');
        if (fs_1.default.existsSync(regularPath) && fs_1.default.existsSync(boldPath)) {
    ...
    doc.text(new Date(analysis.date).toLocaleDateString('tr-TR', {
        day: 'numeric',
        month: 'long',
        year: 'numeric',
    }), 0, doc.y, { align: 'center', width: doc.page.width });
    ...
    const seg = (cnt, color) => {
        ...
        doc.arc(0, 0, r, 0, ang * (Math.PI / 180));
        doc.lineTo(0, 0);
    ...
    const introText = 'Sınava hazırlık sürecinde doğru çalışma modeli ve etkili zaman yönetiminin önemi tartışılmaz bir gerçektir. ' +
    ...
    doc
        ...
        .fontSize(10)
        .fillColor('#374151')
        .text('için eksikleri ve ihtiyaçları dikkate alınarak hazırlanmıştır. Sınav hazırlık sürecinde kolaylıklar, ' +
    ...
    const proj = (0, analysisService_1.simulateImprovement)(analysis.score, analysis.priorityCounts.one, analysis.priorityCounts.two, analysis.priorityCounts.three);
    ...
    doc.text(`1. öncelikli konularını güçlendirdiğinde puanının yaklaşık ${proj.projectedScore.toFixed(1)} seviyesine çıkması beklenmektedir.`, 40, tableTop + 110, { width: 500 });
    ...
    if (topicGroups && topicGroups.rows.length > 0) {
        ...
        for (const row of topicGroups.rows) {
            ...
            const { lessonName, groupName, questionCount, correct, wrong, empty, successPercent, scoreLoss } = row;
    ...
    const y0 = (_a = doc.y) !== null && _a !== void 0 ? _a : 40;
    let y = y0;
    ...
    const priorityOrder = ['ONE', 'TWO', 'THREE'];
    ...
    for (const level of priorityOrder) {
        ...
        doc.text((_b = priorityLabels[level]) !== null && _b !== void 0 ? _b : level, 40, y);
        y += 18;
        ...
        const projLevel = (0, analysisService_1.simulateImprovement)(analysis.score, level === 'ONE' ? list.length : 0, level === 'TWO' ? list.length : 0, level === 'THREE' ? list.length : 0);
        y += 8;
        doc.fontSize(10).fillColor(accentColor);
        doc.text(`Bu gruptaki konular güçlendirildiğinde kayıp puanın yaklaşık ${totalLostLevel.toFixed(1)} puan azaltılabileceği, ` +
            `tahmini puanının ${projLevel.projectedScore.toFixed(1)} seviyesine yaklaşabileceği öngörülmektedir.`, 40, y, { width: 500 });
        y += 20;
    }
    doc.end();
    return done;
